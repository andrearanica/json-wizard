...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print_wizard_error(f"Please provide the path to the configuration JSON file as the first argument.")
        sys.exit(1)

    configuration_json_path = sys.argv[1]
    if not os.path.exists(configuration_json_path):
        print_wizard_error(f"The configuration JSON file at '{configuration_json_path}' does not exist.")
        sys.exit(1)

    if not is_configuration_json_valid(configuration_json_path):
        print_wizard_error(f"The configuration JSON file at '{configuration_json_path}' is not valid.")
        sys.exit(1)

    if len(sys.argv) >= 3:
        destination_json_path = sys.argv[2]
    else:
        print_wizard_warning(f'Destination JSON path not provided; using the default one (\'result.json\' in the current directory).')
